...
public class BPlusTree {
    ...
    public Iterator<RecordId> scanAll() {
        LockUtil.ensureSufficientLockHeld(lockContext, LockType.S);
        Iterator<RecordId> itr = new BPlusTreeIterator();
        return itr;
    }
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        LockUtil.ensureSufficientLockHeld(lockContext, LockType.S);
        Iterator<RecordId> itr = new BPlusTreeIterator(key);
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        if (!ret.isPresent()) {
            ...
            Long retPN = ret.get().getSecond();
            List<DataBox> keys = new ArrayList<>();
            List<Long> children = new ArrayList<>();
            keys.add(retKey);
            children.add(0, root.getPage().getPageNum());
            children.add(1, retPN);
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        if (metadata.getHeight() != 0 || leftMostLeaf.scanAll().hasNext()) { // assert empty
            String msg = "the tree is not empty at time of bulk loding";
            throw new BPlusTreeException(msg);
        }
        ...
        while (data.hasNext()) {
            ...
            if (!ret.isPresent()) {
                ...
                DataBox retKey = ret.get().getFirst();
                long retPN = ret.get().getSecond();
                List<DataBox> newKeys = new LinkedList<>();
                List<Long> newChildren = new LinkedList<>();
                newKeys.add(retKey);
                newChildren.add(root.getPage().getPageNum());
                newChildren.add(retPN);
...
//        List<RecordId> recordIDs = new LinkedList<>();
        private LeafNode nextLeaf; // the leaf that the next rid is in
        private Iterator<RecordId> itr;

        public BPlusTreeIterator() {
            nextLeaf = root.getLeftmostLeaf();
            itr = nextLeaf.scanAll();
        }
        public BPlusTreeIterator(DataBox key) {
            nextLeaf = root.get(key);
            itr = nextLeaf.scanGreaterEqual(key);
        }
...
//            throw new NoSuchElementException();
            RecordId retVal = itr.next();
            if (!itr.hasNext() && nextLeaf.getRightSibling().isPresent()) {
                nextLeaf = nextLeaf.getRightSibling().get();
                itr = nextLeaf.scanAll();
            }
            return retVal;
        }
