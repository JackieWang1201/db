...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        if (keys.size() < 2 * metadata.getOrder()) {
            ...
            if (i < keys.size() && keys.get(i).equals(key)) {
                String msg = "duplicate keys are not allowed";
            ...
            keys.add(i, key);
            rids.add(i, rid);
            List<DataBox> rkeys = keys.subList(d, d * 2 + 1);
            List<RecordId> rrids = rids.subList(d, d * 2 + 1);
            LeafNode newLeaf = new LeafNode(metadata, bufferManager, rkeys, rrids,
                    rightSibling, treeContext);
            for (int k = 2 * d; k >= d; k--) {
                keys.remove(k);
                rids.remove(k);
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        int full = (int) Math.ceil(fillFactor * 2 * metadata.getOrder());
        while (data.hasNext()) {
            ...
            if (keys.size() < full) {
                ...
                rkeys.add(newData.getFirst());
                rrids.add(newData.getSecond());
                LeafNode newLeaf = new LeafNode(metadata, bufferManager, rkeys, rrids,
                        rightSibling, treeContext);
                rightSibling = Optional.of(newLeaf.page.getPageNum());
                newLeaf.sync();
                sync();
                return Optional.of(new Pair<>(rkeys.get(0), newLeaf.getPage().getPageNum()));
            }
        }
        sync();
        return Optional.empty();
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        long rSiblingLong = buf.getLong();
        Optional<Long> rightSibling;
        if (rSiblingLong != -1L) {
            rightSibling = Optional.of(rSiblingLong);
        } else {
