...
class InnerNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (!ret.isPresent()) {
            ...
            if (keys.size() < 2 * d) {
                int i = 0;
                for (; i < keys.size() && keys.get(i).compareTo(retKey) < 0; i++) ;
                keys.add(i, retKey);
                children.add(i + 1, retPN);
                sync();
                ...
                List<DataBox> rkeys = keys.subList(d + 1, d * 2 + 1);
                List<Long> rchildren = children.subList(d + 1, d * 2 + 2);
                InnerNode newInner = new InnerNode(metadata, bufferManager, rkeys, rchildren, treeContext);
                for (int k = 2 * d; k > d; k--) {
                    keys.remove(k);
                    children.remove(k);
                }
