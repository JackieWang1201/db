...
public class ARIESRecoveryManager implements RecoveryManager {
    ...
    public long commit(long transNum) {
        ...
        this.logManager.appendToLog(commitLogRecord);
        //  flush log
        this.logManager.flushToLSN(commitLogRecord.getLSN());
        // update table and status
        Transaction transaction = transEntry.transaction;
        transaction.setStatus(Transaction.Status.COMMITTING);
        transEntry.lastLSN = commitLogRecord.getLSN();
        ///// transactionTable.put(transNum, transEntry);

        return commitLogRecord.getLSN();
    }
    ...
    public long abort(long transNum) {
        ...
        this.logManager.appendToLog(abortLogRecord);

        // update table and status
        Transaction transaction = transEntry.transaction;
        transaction.setStatus(Transaction.Status.ABORTING);
        transEntry.lastLSN = abortLogRecord.getLSN();
        //// transactionTable.put(transNum, transEntry);

        return abortLogRecord.getLSN();
    }
    ...
    public long end(long transNum) {
        ...
        if (transaction.getStatus() == Transaction.Status.ABORTING) {
            while (prevLSN > 0) {
                LogRecord recordToUndo = logManager.fetchLogRecord(prevLSN);
                if (recordToUndo.isUndoable()) {
                    Pair<LogRecord, Boolean> undo = recordToUndo.undo(transEntry.lastLSN);
                    LogRecord CLR = undo.getFirst();
                    this.logManager.appendToLog(CLR);
                    boolean ifFlush = undo.getSecond();
                    transEntry.lastLSN = CLR.getLSN();
                    if (ifFlush) {
                ...
                prevLSN = recordToUndo.getPrevLSN().get();
            }

        }

        // emit end record
        EndTransactionLogRecord endLogRecord = new EndTransactionLogRecord(transNum, transEntry.lastLSN);
        this.logManager.appendToLog(endLogRecord);
        // update table and status
        transaction.setStatus(Transaction.Status.COMPLETE);
        transactionTable.remove(transNum);

        return endLogRecord.getLSN();
    }
    ...
    public long logPageWrite(long transNum, long pageNum, short pageOffset, byte[] before,
        ...
        if (before.length > BufferManager.EFFECTIVE_PAGE_SIZE / 2) {
            UpdatePageLogRecord updatePageLogRecordUndoOnly =
                    new UpdatePageLogRecord(transNum, pageNum, transEntry.lastLSN, pageOffset, before, null);
            this.logManager.appendToLog(updatePageLogRecordUndoOnly);
            transEntry.lastLSN = updatePageLogRecordUndoOnly.getLSN();
            UpdatePageLogRecord updatePageLogRecordRedoOnly =
                    new UpdatePageLogRecord(transNum, pageNum, transEntry.lastLSN, pageOffset, null, after);
            this.logManager.appendToLog(updatePageLogRecordRedoOnly);
            transEntry.lastLSN = updatePageLogRecordRedoOnly.getLSN();
        } else {
            UpdatePageLogRecord updatePageLogRecord =
                    new UpdatePageLogRecord(transNum, pageNum, prevLSN, pageOffset, before, after);
            this.logManager.appendToLog(updatePageLogRecord);
            transEntry.lastLSN = updatePageLogRecord.getLSN();
        ...
        if (!dirtyPageTable.containsKey(pageNum)) {
            dirtyPageTable.put(pageNum, transEntry.lastLSN);
        }

        return transEntry.lastLSN;
    }
    ...
    public void checkpoint() {
        ...
        for (Map.Entry<Long, TransactionTableEntry> entry : transactionTable.entrySet()) {
            ...
            txnTable.put(entry.getKey(), new Pair<>(transEntry.transaction.getStatus(), transEntry.lastLSN));
        }
    ...
    public Runnable restart() {
        ...
        // clean non-dirty pages
        BiConsumer<Long, Boolean> cleanNonDirtyPages = new CleanNonDirtyPages();
        bufferManager.iterPageNums(cleanNonDirtyPages);

        UndoAndCheckpoint undoAndCheckpoint = new UndoAndCheckpoint();

        return undoAndCheckpoint;
    }
    ...
    void restartAnalysis() {
        ...
        while (logItr.hasNext()) {
            LogRecord logRecord = logItr.next();
            if (logRecord.getTransNum().isPresent()) {
                ...
                TransactionTableEntry transEntry = transactionTable.get(transNum);
                transEntry.lastLSN = logRecord.getLSN();
                if (logRecord.getPageNum().isPresent()) {
                    long pageNum = logRecord.getPageNum().get();
                    transEntry.touchedPages.add(pageNum);

                    LockContext pageLockContext = getPageLockContext(pageNum);
                    acquireTransactionLock(transEntry.transaction, pageLockContext, LockType.X);
                    if (!dirtyPageTable.containsKey(pageNum)) {
                        dirtyPageTable.put(pageNum, logRecord.getLSN());
                    }
                    if (logRecord.type.equals(LogType.ALLOC_PAGE) || logRecord.type.equals(LogType.FREE_PAGE)
                            || logRecord.type.equals(LogType.UNDO_ALLOC_PAGE) || logRecord.type.equals(LogType.UNDO_FREE_PAGE)) {
                        dirtyPageTable.remove(pageNum); //?
                    }
                }

                if (logRecord.type.equals(LogType.COMMIT_TRANSACTION)) {
                    transEntry.transaction.setStatus(Transaction.Status.COMMITTING);

                } else if (logRecord.type.equals(LogType.ABORT_TRANSACTION)) {
                    transEntry.transaction.setStatus(Transaction.Status.RECOVERY_ABORTING);

                } else if (logRecord.type.equals(LogType.END_TRANSACTION)) {
            ...
            if (logRecord.type.equals(LogType.BEGIN_CHECKPOINT)) {
            ...
            } else if (logRecord.type.equals(LogType.END_CHECKPOINT)) {
                ...
                for (Map.Entry<Long, Pair<Transaction.Status, Long>> entry : logRecord.getTransactionTable().entrySet()) {
                    long lastLSN_checkpoint = entry.getValue().getSecond();
                    long transNum_table = entry.getKey();
                    TransactionTableEntry transEntry_transTable = transactionTable.get(transNum_table);
                    if (transactionTable.containsKey(entry.getKey())) {
                        if (transEntry_transTable.lastLSN < lastLSN_checkpoint) {
                            transEntry_transTable.lastLSN = lastLSN_checkpoint;
                        }
                    } else {
                        Transaction transaction = newTransaction.apply(transNum_table);
                        transactionTable.put(transNum_table, new TransactionTableEntry(transaction));
                        TransactionTableEntry transEntry = transactionTable.get(transNum_table);
                        transEntry.lastLSN = logRecord.getLSN(); //?
                    }
                ...
                for (Map.Entry<Long, List<Long>> entry : logRecord.getTransactionTouchedPages().entrySet()) {
                    long transNum_table = entry.getKey();
                    if (transactionTable.get(transNum_table).transaction.getStatus().equals(Transaction.Status.COMPLETE)) {
                        continue; //?
                    }
                    for (long pageNum : entry.getValue()) {
                        Set<Long> touchedPages = transactionTable.get(transNum_table).touchedPages;
                        if (!touchedPages.contains(pageNum)) {
                            touchedPages.add(pageNum);
                            LockContext pageLockContext = getPageLockContext(pageNum);
                            acquireTransactionLock(transactionTable.get(transNum_table).transaction, pageLockContext, LockType.X);
                        }
                    }
                }
            }
        ...
        for (Map.Entry<Long, TransactionTableEntry> entry : transactionTable.entrySet()) {
            long transNum_temp = entry.getKey();
            TransactionTableEntry transEntry_temp = entry.getValue();
            if (transEntry_temp.transaction.getStatus().equals(Transaction.Status.COMMITTING)) {
                transEntry_temp.transaction.cleanup();
                transEntry_temp.transaction.setStatus(Transaction.Status.COMPLETE);
                EndTransactionLogRecord endLogRecord = new EndTransactionLogRecord(transNum_temp, transEntry_temp.lastLSN);
                this.logManager.appendToLog(endLogRecord);
                transactionTable.remove(transNum_temp);
            } else if (transEntry_temp.transaction.getStatus().equals(Transaction.Status.RUNNING)) {
                transEntry_temp.transaction.setStatus(Transaction.Status.RECOVERY_ABORTING);
                AbortTransactionLogRecord abortLogRecord = new AbortTransactionLogRecord(transNum_temp, transEntry_temp.lastLSN);
                this.logManager.appendToLog(abortLogRecord);
                transEntry_temp.lastLSN = abortLogRecord.getLSN();
            }
     ...
     */
    void restartRedo() {
        long smallestRecLSN = Long.MAX_VALUE;
        for (Map.Entry<Long, Long> entry : dirtyPageTable.entrySet()) {
        ...
        Iterator<LogRecord> logItr = logManager.scanFrom(smallestRecLSN);
        while (logItr.hasNext()) {
            LogRecord logRecord = logItr.next();
            ...
            if (logRecord.getPageNum().isPresent()) {
                long pageNum = logRecord.getPageNum().get();
                LockContext pageLockContext = getPageLockContext(pageNum);
                Page page = bufferManager.fetchPage(pageLockContext, pageNum, false);
                try {
                    if (!dirtyPageTable.containsKey(pageNum)
                            || logRecord.getLSN() < dirtyPageTable.get(pageNum)
                            || page.getPageLSN() >= logRecord.getLSN()) {
                        continue;
                    }
                } finally {
                    page.unpin(); //test?
    ...
    void restartUndo() {
        ...
        PriorityQueue<Pair<Long, TransactionTableEntry>> toUndo = new PriorityQueue<>(new PairFirstReverseComparator());
        for (Map.Entry<Long, TransactionTableEntry> entry : transactionTable.entrySet()) {
            TransactionTableEntry transEntry = entry.getValue();
            if (transEntry.transaction.getStatus().equals(Transaction.Status.RECOVERY_ABORTING)) {
                toUndo.add(new Pair<>(transEntry.lastLSN, transEntry));
            }
        }
        while (!toUndo.isEmpty()) {
            Pair<Long, TransactionTableEntry> pair = toUndo.poll();
            long LSN = pair.getFirst();
            TransactionTableEntry transEntry = pair.getSecond();
            LogRecord recordToUndo = logManager.fetchLogRecord(LSN);
            if (recordToUndo.isUndoable()) {
                Pair<LogRecord, Boolean> undo = recordToUndo.undo(transEntry.lastLSN);
                LogRecord CLR = undo.getFirst();
                this.logManager.appendToLog(CLR);
                ...
                if (!dirtyPageTable.containsKey(CLR.getPageNum())) {
                    dirtyPageTable.put(CLR.getPageNum().get(), CLR.getLSN());
                }
            }
            if (recordToUndo.getUndoNextLSN().isPresent()) {
                toUndo.add(new Pair<>(recordToUndo.getUndoNextLSN().get(), transEntry));
            } else if (recordToUndo.getPrevLSN().isPresent()) {
                long prevLSN = recordToUndo.getPrevLSN().get();
                if (prevLSN != 0) {
                    toUndo.add(new Pair<>(recordToUndo.getPrevLSN().get(), transEntry));
                } else {
                    long transNum = transEntry.transaction.getTransNum();
                    EndTransactionLogRecord endLogRecord = new EndTransactionLogRecord(transNum, transEntry.lastLSN);
                    this.logManager.appendToLog(endLogRecord);
                    transEntry.transaction.setStatus(Transaction.Status.COMPLETE);
                    transactionTable.remove(transNum);
    ...
    class CleanNonDirtyPages implements BiConsumer<Long, Boolean> {
        @Override
    ...
    class UndoAndCheckpoint implements Runnable {
        public void run() {
            restartUndo();
            checkpoint();
        }
