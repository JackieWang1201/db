...
public class QueryPlan {
    ...
    public Iterator<Record> execute() {
        ...
        try {
            ...
            Map<Set, QueryOperator> map;
            Set<String> tables = new HashSet<>();
            tables.add(startTableName);
            pass1Map.put(tables, minCostSingleAccess(startTableName));
            for (String table: joinTableNames) {
                tables = new HashSet<>();
                tables.add(table);
                pass1Map.put(tables, minCostSingleAccess(table));
            }
            map = pass1Map;
            int numTables = 1 + joinTableNames.size();
    ...
    QueryOperator minCostSingleAccess(String table) {
        ...
        ////List<String> indexColumns = getAllIndexColumns(table);
        List<Integer> selectIndices = getEligibleIndexColumns(table);
        for (int index : selectIndices) {
            String columnName = selectColumnNames.get(index);
            PredicateOperator predicate = selectOperators.get(index);
            DataBox value = selectDataBoxes.get(index);
            QueryOperator indexScanOp =
                    new IndexScanOperator(this.transaction, table, columnName, predicate, value);
    ...
    Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
         ...
         */
        QueryOperator minCostJoinOp;
        for (Set prevTables : prevMap.keySet()) {
            for (String joinTableName : this.joinTableNames) {
                int index = joinTableNames.indexOf(joinTableName);
                String[] left = getJoinLeftColumnNameByIndex(index);
                String[] right = getJoinRightColumnNameByIndex(index);
                ...
                if (prevTables.contains(leftTable) && !prevTables.contains(rightTable)) {
                    ...
                    minCostJoinOp = minCostJoinType(prevMap.get(prevTables), rightOp, leftColumn, rightColumn);
                    Set<String> newTables = new HashSet(prevTables);
                    newTables.add(rightTable);
                    if (map.containsKey(newTables)) {
                        int oldCost = map.get(newTables).getIOCost();
                        int newCost = minCostJoinOp.getIOCost();
                        if (newCost < oldCost) {
                            map.put(newTables, minCostJoinOp);
                        }
                ...
                } else if (!prevTables.contains(leftTable) && prevTables.contains(rightTable)) {
                    Set<String> tableSet = new HashSet<>();
                    tableSet.add(leftTable);
                    QueryOperator leftOp = pass1Map.get(tableSet);
                    minCostJoinOp = minCostJoinType(prevMap.get(prevTables), leftOp, rightColumn, leftColumn);
                    Set<String> newTables = new HashSet(prevTables);
