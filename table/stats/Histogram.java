...
public class Histogram {
    ...
    public void buildHistogram(Table table, int attribute) {
        ...
        for (Record record : table) {
            if (quantization(record, attribute) < minValue) {
                minValue = quantization(record, attribute);
            }
            if (quantization(record, attribute) > maxValue) {
                maxValue = quantization(record, attribute);
            }
        }
        ...
        for (int i = 0; i < buckets.length; i++) {
            float start, end;
            start = minValue + i * width;
            end = minValue + (i + 1) * width;
        ...
        for (Record record : table) {
            ...
            int bIndex = bucketIndex(value);
            buckets[bIndex].increment(value);
        }
    ...
    private float [] allEquality(float qvalue) {
        ...
        int bIndex = bucketIndex(qvalue);
        for (int i = 0; i < this.buckets.length; i++) {
            if (i == bIndex) {
                int distinctCount = get(bIndex).getDistinctCount();
                result[i] = 1.0f / distinctCount;
            } else {
                result[i] = 0f;
    ...
    private float [] allNotEquality(float qvalue) {
        ...
        for (int i = 0; i < this.buckets.length; i++) {
            if (i == bIndex) {
                int distinctCount = get(bIndex).getDistinctCount();
                result[i] = 1.0f - 1.0f / distinctCount;
            } else {
                result[i] = 1.0f;
