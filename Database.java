...
public class Database implements AutoCloseable {
    ...
    void lockTableMetadata(String tableName, LockType lockType) {
        ...
        if (mayNeedToCreate) {
            LockUtil.ensureLocksAbovePage(getTableInfoContext(), lockType);
        ...
        LockContext pageContext = getTableInfoContext().childContext(tableInfoLookup.get(tableName).getPageNum()); //?
    ...
    void lockIndexMetadata(String indexName, LockType lockType) {
        ...
        if (mayNeedToCreate) {
            LockUtil.ensureLocksAbovePage(getIndexInfoContext(), lockType);
        ...
        LockContext pageContext = getIndexInfoContext().childContext(indexInfoLookup.get(indexName).getPageNum()); //?
        LockUtil.ensureSufficientLockHeld(pageContext, lockType);
    }
    ...
    private class TransactionContextImpl extends AbstractTransactionContext {
        ...
        public Table getTable(String tableName) {
            ...
            lockTableMetadata(tableName, LockType.S);
            //lockIndexMetadata(tableName, LockType.S);
    ...
    private class TransactionImpl extends AbstractTransaction {
        ...
        public void createIndex(String tableName, String columnName, boolean bulkLoad) {
            ...
            try {
                ...
                for (String indexName : tableIndices.get(prefixedTableName)) {
        ...
        public void dropIndex(String tableName, String columnName) {
            ...
            TransactionContext.setTransaction(transactionContext);
            try {
                for (String index_name : tableIndices.get(prefixedTableName)) {

                    lockIndexMetadata(index_name, LockType.X);
                }
